...
@section('title', 'Create Trivia Question')
        ...
        <div class="col-md-8">
            <div class="card shadow">
                <div class="card-header bg-success text-white">
                    <h4 class="mb-0"><i class="fa fa-plus-circle"></i> Create Trivia Question</h4>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.trivia.store') }}">
                        @csrf
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                                        ...
                                        <option value="text" {{ old('type') == 'text' ? 'selected' : '' }}>Text Answer</option>
                                    </select>
                                    @error('type')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control @error('points') is-invalid @enderror"
                                           id="points" name="points" value="{{ old('points') }}" min="1" max="100" required>
                                    @error('points')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="option-group mb-2">
                                    <div class="input-group">
                                        <div class="input-group-text">
                                            <input class="form-check-input mt-0" type="radio" name="correct_answer" value="{{ $i }}" {{ old('correct_answer') == $i ? 'checked' : '' }} required>
                                        </div>
                                        <input type="text" class="form-control" name="options[{{ $i }}][text]"
                                               placeholder="Option {{ $i + 1 }}" value="{{ old('options.' . $i . '.text') }}" required>
                                    </div>
                                </div>
                                @endfor
                            </div>
                        ...
                        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                            <a href="{{ route('admin.trivia.index') }}" class="btn btn-secondary me-md-2">
